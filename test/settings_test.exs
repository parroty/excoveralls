defmodule Excoveralls.SettingsTest do
  ...
  @fixture_custom Path.dirname(__ENV__.file) <> "/fixtures/custom.exs"
  @fixture_dotfile Path.dirname(__ENV__.file) <> "/fixtures/dotfile.exs"
  @fixture_invalid Path.dirname(__ENV__.file) <> "/fixtures/invalid.exs"
  @fixture_not_covered Path.dirname(__ENV__.file) <> "/fixtures/no_relevant_lines_not_covered.exs"
  @fixture_covered Path.dirname(__ENV__.file) <> "/fixtures/no_relevant_lines_are_covered.exs"
  @fixture_column_default Path.dirname(__ENV__.file) <> "/fixtures/terminal_options1.exs"
  @fixture_column_integer Path.dirname(__ENV__.file) <> "/fixtures/terminal_options2.exs"
  @fixture_no_summary Path.dirname(__ENV__.file) <> "/fixtures/no_summary.exs"

  test "returns default file path" do
    assert(
      Settings.Files.default_file()
      |> Path.relative_to(File.cwd!()) == "lib/excoveralls/../conf/coveralls.exs"
    )
  end

  test "returns custom file path" do
    assert(
      Settings.Files.custom_file()
      |> Path.relative_to(File.cwd!()) == "coveralls.exs"
    )
  end

  test "returns custom file path from config file" do
    Application.put_env(:excoveralls, :config_file, "custom_coveralls.exs")

    assert(
      Settings.Files.custom_file()
      |> Path.relative_to(File.cwd!()) == "custom_coveralls.exs"
    )
  ...
  test_with_mock "read config returns default value when custom file is not found",
                 Settings.Files,
                 default_file: fn -> @fixture_default end,
                 custom_file: fn -> "__invalid__" end,
                 dot_file: fn -> @fixture_dotfile end do
  ...
  test_with_mock "read config fails if exs file is invalid", Settings.Files,
    default_file: fn -> @fixture_default end,

defmodule Excoveralls.SettingsTest do
  ...
  test "returns default file path" do
    assert(
      ...
      |> Path.relative_to(File.cwd!()) == "lib/chaps/../conf/coveralls.json"
    )
  end
  ...
  test "returns custom file path from config file" do
    Application.put_env(:chaps, :config_file, "custom_coveralls.json")
    ...
    Application.delete_env(:chaps, :config_file)
  end
  ...
  test_with_mock "get stop words returns merged default and custom stop words",
                 Settings.Files,
                 default_file: fn -> @fixture_default end,
                 custom_file: fn -> @fixture_custom end,
                 dot_file: fn -> "__invalid__" end do
  ...
  test_with_mock "read config returns nil when default file is not found",
                 Settings.Files,
  ...
  test_with_mock "merges dotfile config with custom config",
    ...
    assert Settings.get_stop_words() == [
             ~r/a/,
             ~r/b/,
             ~r/cc/,
             ~r/dd/,
